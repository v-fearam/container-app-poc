...
22. **Cosmos SDK NO actualiza campos custom de timestamp automáticamente:** `updatedAt` es un campo custom que DEBE setearse manualmente en `CreateItemAsync` y `ReplaceItemAsync`. El SDK solo actualiza `_ts` (sistema). Sin setear `updatedAt` manualmente, el Change Feed Processor puede ignorar cambios si usás comparación de timestamps (línea 83 de `ChangeFeedHandler.cs` usa `>=` para permitir timestamps iguales). **Solución:** Siempre `updated.UpdatedAt = DateTime.UtcNow` antes de guardar. Ver comentarios en `CosmosPersonasController.cs` líneas 131 y 183.
...
27. **Token Store SAS secret no existe en deploy inicial:** `enableTokenStore=true` hardcodeado causa que el frontend falle con `"Unable to get value using Managed identity... for secret token-store-sas"` porque el secret se crea recién en Paso 7 (Easy Auth). **Solución:** `enableTokenStore` es param en `main.bicep` con default `false`. Pasar `enableTokenStore=true` solo DESPUÉS de Paso 7 (cuando `token-store-sas` ya existe en Key Vault).
