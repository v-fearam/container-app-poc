...
18. **Cosmos connection string no auto-seed:** Bicep NO puede auto-crear el secret `cosmos-connection-string` en Key Vault porque genera un ciclo de dependencias (KeyVault → Cosmos → WorkerIdentity → KeyVault). **Solución:** Agregar manualmente después del deploy de infra (Paso 4.1 en DEPLOYMENT.md). Sin este secret: backend devuelve **500 Internal Server Error** en todos los endpoints de Cosmos (`/api/cosmos/personas`, `/api/dashboard/kpi`) con error `"Unable to resolve service for type 'Microsoft.Azure.Cosmos.CosmosClient'"`.
19. **Easy Auth secrets DEBEN estar en Key Vault ANTES del deployment:** Los Container Apps (frontend y backend) tienen `enableAuth: true` y referencian `auth-client-secret-frontend` y `auth-client-secret-backend` desde Key Vault. Si estos secrets no existen al hacer el deployment de Container Apps (Paso 6), el deployment **falla con error de referencia a secret inexistente** o los apps no arrancan. **Solución:** Ejecutar Paso 2 (agregar secrets) ANTES del Paso 6 (deploy Container Apps).
20. **Backend identity necesita CREATE USER en SQL Database:** Después del deployment, el backend tiene managed identity con RBAC roles en Azure, pero **NO** tiene usuario SQL dentro de la database. Sin el `CREATE USER` manual, el backend falla al conectarse a SQL con error `"Login failed for user '<token-identified principal>'"`. **Solución:** Ejecutar Paso 5.1 para crear usuarios SQL para AMBOS backend identity Y worker identity. Bicep NO puede automatizar esto (limitación de Azure SQL, no de Bicep).
